...
try:
    ...
    active_sessions = directory_df["Session"].dropna().astype(str).tolist()
...
# --- 6. SESSION-SPECIFIC VOTE INFORMATION ---
session_votes = votes_df[votes_df["Session"] == selected_session].copy()

voted_users = sorted(session_votes["Username"].dropna().astype(str).unique().tolist())

has_voted = current_user in voted_users


def build_results_table(session_votes_df):
    """Return aggregated results for one session."""
...
# --- 7. SHOW VOTING PROGRESS TO EVERYONE ---
...
if session_status == "CLOSED":
    ...
    st.header("📊 Current Live Results")

    live_results = build_results_table(session_votes)
    ...
    if has_voted:
        ...
        try:
            ...
            if "Book" not in books_df.columns:
                st.error(
                    f"The worksheet '{selected_session}' must contain a 'Book' column."
                )
                st.stop()
        ...
        if submitted:
            ...
            # Re-read votes right before writing, to reduce overwrite risk
            try:
            ...
            latest_voted_users = (
                latest_session_votes["Username"].dropna().astype(str).unique().tolist()
            )
